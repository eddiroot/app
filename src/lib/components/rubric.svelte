...
<div class="bg-background overflow-hidden rounded-lg border shadow-sm">
	...
	<div class="bg-muted/30 border-b p-6">
		...
		{#if rubricData.rubric.description}
			<p class="text-muted-foreground mt-2 text-sm">{rubricData.rubric.description}</p>
		{/if}
	</div>
						...
						<div class="bg-card min-h-[100px] rounded-lg border p-3">
							{#if cell}
								<div class="space-y-2">
									...
									<div class="text-foreground/70 mt-2 border-t pt-2 text-xs font-medium">
										{cell.marks}
										{cell.marks === 1 ? 'mark' : 'marks'}
									</div>
								</div>
							{:else}
								<p class="text-muted-foreground text-center text-sm italic">Not defined</p>
							{/if}
						</div>
					{/each}
				</div>
