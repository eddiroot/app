...
console.log(
	`${timetable.userName} has ${timetable.totalHoursPerCycle}h per cycle`,
);
...
```typescript
const timetables = await generateMultipleUserTimetables(
...
for (let week = 0; week < weeksInCycle; week++) {
	const weekDays = userTimetable.days.slice(
		week * daysPerWeek,
		(week + 1) * daysPerWeek,
	);
