...
export async function generateTimetableStatistics(
	...
	return { students, teachers, summary }
}
...
function calculateTimetableSummary(
	...
	const averageStudentHoursPerCycle =
		studentHours.length > 0
			? studentHours.reduce((a, b) => a + b, 0) / studentHours.length
			: 0
	const averageTeacherHoursPerCycle =
		teacherHours.length > 0
			? teacherHours.reduce((a, b) => a + b, 0) / teacherHours.length
			: 0

	const studentWorkloadDistribution =
		calculateWorkloadDistribution(studentHours)
	const teacherWorkloadDistribution =
		calculateWorkloadDistribution(teacherHours)
...
function calculateDayUtilization(
	...
	for (const dayUtil of dayMap.values()) {
		dayUtil.averageStudentHours =
			dayUtil.studentsScheduled > 0
				? dayUtil.totalStudentHours / dayUtil.studentsScheduled
				: 0
		dayUtil.averageTeacherHours =
			dayUtil.teachersScheduled > 0
				? dayUtil.totalTeacherHours / dayUtil.teachersScheduled
				: 0
	}
...
export function identifyWorkloadIssues(
	...
	const underutilized = students.filter(
		(s) => s.averageHoursPerDay < minHoursPerDay,
	)
	...
	return { overloaded, underutilized, unbalanced }
}
...
export function identifyTeacherWorkloadIssues(
	...
	const underutilized = teachers.filter(
		(t) => t.averageHoursPerDay < minHoursPerDay,
	)
...
export function generateStatisticsReport(stats: TimetableStatistics): string {
	...
	lines.push('-'.repeat(60))
	lines.push(
		`Average Hours/Cycle: ${stats.summary.averageStudentHoursPerCycle.toFixed(2)}h`,
	)
	lines.push(
	...
	lines.push('-'.repeat(60))
	lines.push(
		`Average Hours/Cycle: ${stats.summary.averageTeacherHoursPerCycle.toFixed(2)}h`,
	)
	lines.push(
