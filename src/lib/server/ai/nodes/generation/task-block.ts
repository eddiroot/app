...
import { buildBlockUnionSchema, getBlockTypesForSubjectGroup, taskBlockSchema } from "../../schemas/task-block";
...
export function multiBlockGenerationNode(llm: BaseChatModel) {
    return async (input: BlockGenerationInput): Promise<BlockGenerationOutput> => {
        ...
        // Build prompt
        const sectionContent = "Scope: " + input.contentSection.scope + "\n\nContent: " + input.contentSection.content;
        const prompt = buildTaskBlockPrompt(
            blockTypes,
            input.taskType,
            sectionContent
        );
