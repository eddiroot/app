...
function buildSubjectGroupPrompt(subjectName: string): string {
    const groupDescriptions = {
        [subjectGroupEnum.mathematics]: "Mathematics, Maths, Calculus, Algebra, Geometry, Statistics, Numeracy, Further Maths, Mathematical Methods, Specialist Maths",
        [subjectGroupEnum.science]: "Science, Physics, Chemistry, Biology, Environmental Science, Earth Science, Psychology, Science subjects",
        [subjectGroupEnum.english]: "English, Literature, Language Arts, Writing, Reading, Creative Writing, English Language, EAL, History, Geography, Economics, Business, Legal Studies, Politics, Philosophy, Sociology, Languages, Arts, Music, Health, PE, Technology"
    };
...
export function quickSubjectGroupLookup(subjectName: string): subjectGroupEnum | undefined {
    ...
    // Mathematics
    if (/\b(math|maths|mathematics|calculus|algebra|geometry|statistics|numeracy|further.*math|mathematical.*methods|specialist.*math)\b/i.test(normalizedName)) {
        return subjectGroupEnum.mathematics;
    }
    
    // Science
    if (/\b(science|physics|chemistry|biology|environmental|earth.*science|psychology)\b/i.test(normalizedName)) {
        return subjectGroupEnum.science;
    }
    
    // English (and everything else - humanities, languages, arts, technology, health, etc.)
    if (/\b(english|literature|language|creative.*writing|eal|history|geography|economics|business|legal|politics|philosophy|sociology|religion|civics|french|spanish|german|japanese|chinese|italian|indonesian|latin|greek|art|arts|visual.*art|music|drama|theatre|dance|media|film|design|photography|technology|computing|computer|it|digital|software|programming|engineering|robotics|systems|health|physical.*education|pe|sport|outdoor|food.*tech|hhd|human.*development)\b/i.test(normalizedName)) {
