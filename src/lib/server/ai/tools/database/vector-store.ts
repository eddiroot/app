...
export function createVectorStoreTool<T extends Record<string, unknown>>(
	...
	return tool(
		...
		async ({ query, limit }) => {
			...
			return await vectorStore.similaritySearchAsDocuments(query, limit ?? k ?? 5);
		},
		{
			name,
			description,
	...
	return tool(
		...
		async ({ query, limit }) => {
			...
			for (const vectorStore of vectorStores) {
				const storeResults = await vectorStore.searchWithScores(queryEmbed, perStoreLimit, filter);
