	...
	async (input: TaskCreationData) => {
		...
		if (input.plan) {
			if (input.taskType === taskTypeEnum.lesson) {
				newPlan = await createCourseMapItemLessonPlan(
					input.plan.courseMapItemId,
					input.plan.name,
					input.plan.scopes,
				...
				await Promise.all(
					input.plan.learningAreaStandardIds.map(async (standardId) => {
						await createLessonPlanStandard(newPlan!.id, standardId);
					})
				);
			} else if (
				input.taskType === taskTypeEnum.test ||
				input.taskType === taskTypeEnum.assignment
			) {
				newPlan = await createCourseMapItemAssessmentPlan(
					input.plan.courseMapItemId,
					input.plan.name,
					input.plan.scopes,
					input.plan.description
