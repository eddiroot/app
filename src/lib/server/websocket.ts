...
export function setupWebSocketServer(httpServer: HTTPServer | Http2SecureServer) {
	...
	io.on('connection', (socket) => {
		...
		socket.on('init', async (data: { whiteboardId: number }) => {
			try {
				...
				console.log(`Client ${socket.id} initializing whiteboard ${whiteboardId}`);

				// Leave previous room if any
				if (currentWhiteboardId) {
					socket.leave(`whiteboard-${currentWhiteboardId}`);
					console.log(`Client ${socket.id} left whiteboard-${currentWhiteboardId}`);
				}
				...
				console.log(`Sending ${whiteboardObjects.length} objects to client ${socket.id}`);
				socket.emit('load', {
					whiteboardId,
		...
		socket.on('add', async (data: { whiteboardId: number; object: any }) => {
			try {
				...
				console.log(`Adding object ${newObject.id} to whiteboard ${whiteboardId}`);
				...
				socket.broadcast.to(`whiteboard-${whiteboardId}`).emit('add', {
					whiteboardId,
					object: newObject
				});
		...
		socket.on('modify', async (data: { whiteboardId: number; object: any; live?: boolean }) => {
			try {
				...
				// Broadcast to all clients EXCEPT the sender
				socket.broadcast.to(`whiteboard-${whiteboardId}`).emit('modify', {
					whiteboardId,
		...
		socket.on('delete', async (data: { whiteboardId: number; objects?: any[]; object?: any }) => {
			try {
				const { whiteboardId, objects: objectsToRemove, object: singleObject } = data;
				...
				if (objectsToRemove) {
					...
					// Broadcast to all clients EXCEPT the sender
					socket.broadcast.to(`whiteboard-${whiteboardId}`).emit('delete', {
						whiteboardId,
		...
		socket.on('clear', async (data: { whiteboardId: number }) => {
			try {
				...
				// Broadcast to all clients EXCEPT the sender
				socket.broadcast.to(`whiteboard-${whiteboardId}`).emit('clear', { whiteboardId });
			} catch (error) {
		...
		socket.on('layer', async (data: { whiteboardId: number; object: any; action: string }) => {
			try {
				...
				// Broadcast to all clients EXCEPT the sender
				socket.broadcast.to(`whiteboard-${whiteboardId}`).emit('layer', {
					whiteboardId,
		...
		socket.on('lock', async (data: { whiteboardId: number; isLocked: boolean }) => {
			try {
		...
		socket.on('unlock', async (data: { whiteboardId: number; isLocked: boolean }) => {
			try {
				const { whiteboardId, isLocked } = data;
