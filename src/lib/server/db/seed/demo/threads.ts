import { subjectThreadResponseTypeEnum, subjectThreadTypeEnum } from '$lib/enums';
...
const THREAD_TEMPLATES: Record<string, Record<string, { title: string; content: string }[]>> = {
	Mathematics: {
		discussion: [
			{
...
const TEACHER_ANNOUNCEMENTS: Record<string, { title: string; content: string }[]> = {
	Mathematics: [
		{
...
export async function seedDemoThreads(
	...
	const { offerings, subjects, coreSubjects } = subjectData;
	...
	// Process a subset of offerings (semester 1 only to avoid duplicates)
	const semester1Offerings = offerings.filter((o) => o.semester === 1);

	for (const offering of semester1Offerings) {
		...
		for (let i = 0; i < numDiscussions && i < templates.discussion.length; i++) {
			...
			for (const responder of responders) {
				const responseType =
					random() < 0.3 ? 'agreement' : random() < 0.6 ? 'discussion' : 'encouragement';
			...
			for (const liker of likers) {
				await db.insert(schema.subjectThreadLike).values({
					subjectThreadId: thread.id,
					userId: liker.id
				});
				likeCount++;
		...
		for (let i = 0; i < numQuestions && i < templates.question.length; i++) {
			...
			for (let j = 0; j < responders.length; j++) {
				...
				const responseType = j === 0 ? 'helpful' : random() < 0.5 ? 'helpful' : 'followUp';
				const responseContent =
					RESPONSE_TEMPLATES[responseType][
						Math.floor(random() * RESPONSE_TEMPLATES[responseType].length)
					];
				...
				if (responseType === 'helpful') {
					...
					const likers = shuffle(shuffledStudents, random).slice(0, numLikes);
					for (const liker of likers) {
						await db.insert(schema.subjectThreadResponseLike).values({
		...
		if (teachersInOffering.length > 0 && announcements.length > 0) {
			...
			if (random() < 0.6) {
				...
				const commenters = shuffle(shuffledStudents, random).slice(0, numComments);
				...
				for (const commenter of commenters) {
					const comment = studentComments[Math.floor(random() * studentComments.length)];
					await db.insert(schema.subjectThreadResponse).values({
						type: subjectThreadResponseTypeEnum.comment,
						subjectThreadId: thread.id,
						userId: commenter.id,
