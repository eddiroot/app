...
export async function getNewsById(
	...
	const newsItems = await db
		...
		.leftJoin(
			table.newsCategory,
			eq(table.news.categoryId, table.newsCategory.id),
		)
		.leftJoin(
			table.schoolCampus,
			eq(table.news.schoolCampusId, table.schoolCampus.id),
		)
		.where(
			includeArchived
...
export async function getPublishedNewsBySchoolId(
	...
	if (userType) {
		...
		if (
			['student', 'teacher', 'guardian', 'principal', 'schoolAdmin'].includes(
				userType,
			)
		) {
...
export async function getNewsResources(
	...
	const resources = await db
		.select({ newsResource: table.newsResource, resource: table.resource })
		.from(table.newsResource)
		.innerJoin(
			table.resource,
			eq(table.newsResource.resourceId, table.resource.id),
		)
		.where(
			and(
				...
				or(
					sql`${table.resource.fileType} LIKE 'image/%'`,
