...
export async function getBehavioursBySchoolId(schoolId: number) {
	const results = await db
		.select({
			behaviour: table.schoolBehaviour,
			behaviourLevel: table.schoolBehaviourLevel,
		})
		.from(table.schoolBehaviour)
		.leftJoin(
			table.schoolBehaviourLevel,
			eq(table.schoolBehaviour.levelId, table.schoolBehaviourLevel.id),
		)
		.where(
			and(
				eq(table.schoolBehaviour.schoolId, schoolId),
				eq(table.schoolBehaviour.isArchived, false),
			),
		)
		.orderBy(
...
export async function getLevelsWithBehaviours(schoolId: number) {
	const results = await db
		.select({
			level: table.schoolBehaviourLevel,
			behaviour: table.schoolBehaviour,
		})
		.from(table.schoolBehaviourLevel)
		.leftJoin(
...
export async function getBehavioursByAttendanceId(attendanceId: number) {
	const behaviours = await db
		.select({ behaviour: table.schoolBehaviour })
