...
import type { CurriculumSubject, LearningArea, LearningAreaStandard, LearningAreaTopic, StandardElaboration } from '$lib/server/db/schema/curriculum';
...
export async function getCurriculumDataForCurriculumSubjectId(curriculumSubjectId: number, yearLevel?: yearLevelEnum): Promise<CurriculumSubjectData | null> {
	...
	const buildStandardWithElaborations = (standard: typeof standards[0]): LearningStandardsWithElaborations => ({
	...
	const learningAreasData: LearningAreaData[] = learningAreas.map((la) => {
		...
		return {
			...
			topics: topicsForArea.map(topic => {
				const standardsForTopic = standardsForArea.filter(s => s.learningAreaTopicId === topic.id);
				return {
					topic,
