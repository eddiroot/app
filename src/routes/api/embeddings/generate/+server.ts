...
async function generateEmbeddingsForTable(
  ...
  try {
    ...
    console.log(`📋 ${tableName}: Found ${stats.total} records to process (batches of ${BATCH_SIZE} texts per API call)`);
    ...
    for (let i = 0; i < records.length; i += BATCH_SIZE) {
      ...
      console.log(`⚙️  ${tableName}: Batch ${batchNum}/${totalBatches} (${batch.length} texts in 1 API call)...`);
      ...
      try {
        ...
        if (stats.failed <= batch.length * 3) {
          console.error(`❌ ${tableName} batch ${batchNum} failed: ${errorMessage.substring(0, 100)}`);
        }
      }
