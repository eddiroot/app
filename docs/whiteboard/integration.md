...
export async function getWhiteboardWithTask(whiteboardId: number, taskId: number) {
  const [result] = await db
    ...
    .where(
      and(
        eq(table.whiteboard.id, whiteboardId),
...
```typescript
export async function createTaskBlock(taskId: number, type: TaskBlockType, config?: any) {
...
export async function saveWhiteboardObject(
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [table.whiteboardObject.whiteboardId, table.whiteboardObject.objectId],
...
```typescript
export async function deleteWhiteboardObject(whiteboardId: number, objectId: string) {
  await db
...
<div class="whiteboard-block">
  ...
  <a href={whiteboardUrl} class="btn">
    Open Whiteboard
  </a>
</div>
```
